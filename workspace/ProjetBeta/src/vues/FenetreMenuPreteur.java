...
public class FenetreMenuPreteur extends JFrame {
	private static final long serialVersionUID = -4163074205881297312L;

	// Pseudo en paramètre pour récupérer celui de la connexion
	public static void menuPreteur(Preteur preteur) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FenetreMenuPreteur frame = new FenetreMenuPreteur(preteur);
	...
	public FenetreMenuPreteur(Preteur preteur) {
		setTitle("Votre menu");
		setResizable(false);
		...
		btnVoirReservation.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				dispose();
				FenetrePretPreteur.main(preteur);
			}
		});
		...
		btnAjouterUnJeu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				dispose();
				FenetreAjoutJeu.main(preteur);
			}
		});
		btnAjouterUnJeu.setFont(new Font("Century Gothic", Font.BOLD, 15));
		btnAjouterUnJeu.setBackground(Color.WHITE);
		toolBar.add(btnAjouterUnJeu);
